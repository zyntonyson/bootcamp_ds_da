@countdown{timer: 180 , title_transition: "¡Comenzamos en breve!"}

# Sprint 8 Sesión 1 
## Explorar conexiones de datos con correlaciones 


---

@warnup-mood{title_transition: "Midamos los ánimos"}

# ¿Cómo llegas a la sesión de hoy?
## Reacciona con el emoji 😎🤔🤓🙂 que mejor represente tu mood

---

@agenda{title_transition: "Nuestra Agenda de Hoy"}

# Agenda de la sesión

* 👋 Bienvenida {5}
* 👋 Avisos parroquiales {5}
* 📚 Revisión de conceptos {10}
* 💼 Caso de estudio: German city Bank {40}
* 🤔 Reflexión y discusión: ¿Qué aprendimos en esta sesión? {5}


---
@two_columns_slide{title_transition: "Avisos parroquiales"}



.left{

## Estaré de viaje del 28 de abril al 3 de mayo

![Viaje](https://raw.githubusercontent.com/zyntonyson/img_repo/refs/heads/main/img/de_viaje.png)
}

.right{

- *Vamos a tener unos ajustes en el calendario*:

- **DA-82**: Se mueve del jueves 30 de abril al lunes 27 de abril a las 8:00 PM (hora de CDMX)
- **DA-98**: Se mueve del jueves 30 de abril al lunes 4 de mayo a las 8:00 PM (hora de CDMX)
- **DA-104**: Se mueve del miércoles 29 de abril al lunes 27 de abril a las 8:00 PM (hora de CDMX)


}

---


@objectives{title_transition: "¿Qué vamos a practicar?"}

# Objetivos académicos de esta clase

 
* 📈 Calcular métricas de correlaciones
* 🧠 Interpretar resultados de análisis de correlación
* ⚖️ Integrar pensamiento ético en el análisis de correlación


---

@gotocode{title_transition: "Manos a la Obra"}

# Escanea el código para los materiales de la clase



* [Sprint 8 · Explorar conexiones de datos con correlaciones I](../img/qrs/23-da-v8-analizar-correlaciones.png )

---

@finale{}

# ¡Gracias inmensas!

## Tu asistencia y participación hacen que la clase sea muy valiosa
